...
import type { CmsSiteLinks } from "@/lib/cms.types";
...
function resolveHref(hrefKey: string, links: CmsSiteLinks) {
  if (hrefKey in links) {
    return links[hrefKey as keyof CmsSiteLinks];
  }
  return hrefKey;
}
...
export function HireWithConfidence() {
  ...
  const hire = sections.hire;

  return (
    <section id="hire" className="relative overflow-hidden py-12 sm:py-16">
      <div className="mx-auto max-w-6xl px-5 sm:px-8">
        <SectionHeading eyebrow={hire.eyebrow} title={hire.title} subtitle={hire.subtitle} />
        ...
        <Reveal className="relative mx-auto mt-9 max-w-4xl">
          ...
          <div className="relative grid gap-4 rounded-[1.75rem] border border-white/10 bg-white/[0.025] p-3 shadow-[0_28px_80px_-50px_rgba(0,0,0,0.9)] backdrop-blur-sm sm:grid-cols-2 sm:p-4">
            {hire.platforms.map((platform, i) => {
              const href = resolveHref(platform.hrefKey, settings.links);
              return (
